<section id='content_profile'>
    <div>
        ...
        <?php
        $itemsPerPage = 5; // Número de consultas por página
        $page_num = isset($_GET['page_num']) ? (int)$_GET['page_num'] : 1; // Página atual
        $totalAppointments = count($_SESSION['appointments']); // Total de consultas
        $totalPages = ceil($totalAppointments / $itemsPerPage); // Total de páginas
        $start = ($page_num - 1) * $itemsPerPage; // Índice inicial

        // Obtenha as consultas da página atual
        $appointmentsPage = array_slice($_SESSION['appointments'], $start, $itemsPerPage);
        ?>

        <fieldset class="perfil">
            <?php if ($_SESSION['role_user'] == 'paciente') { // Corrigido o sinal de comparação 
            ?>
